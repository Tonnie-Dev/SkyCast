    ...
    constructor(
        ...
        override fun getForecastWeather(cityId: Int): Flow<Resource<List<ForecastWeather>>> =
            flow {
                ...
                fetchLocalForecastWeather()
                    ?.takeIf { !it.isExpired() && it.isNotEmpty() }
                    ?.let {
        ...
        private suspend fun fetchLocalCurrentWeather(): CurrentEntity? = localDataSource.getCurrentWeather()

        private suspend fun fetchLocalForecastWeather(): List<ForecastEntity>? = localDataSource.getForecastWeather()

        private fun CurrentEntity.isExpired(): Boolean = System.currentTimeMillis() - this.lastFetchTime > EXPIRY_TIME

        private fun List<ForecastEntity>.isExpired(): Boolean =
            if (isEmpty()) true else System.currentTimeMillis() - this.first().lastFetchTime > EXPIRY_TIME
        ...
        ): Flow<Resource<List<ForecastWeather>>> =
            flow {
                emit(Resource.Loading(isLoading = true))
